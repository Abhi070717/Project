...
@WebServlet(name = "VaccineCtl", urlPatterns = { "/VaccineCtl" })
public class VaccineCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("vaccineName"))) {
			request.setAttribute("vaccineName", PropertyReader.getValue("error.require", "Vaccine Name"));
			pass = false;
		}
		if (DataValidator.isNull(request.getParameter("manufacturer"))) {
			...
			pass = false;
		} else if (!DataValidator.isName(request.getParameter("manufacturer"))) {
			request.setAttribute("manufacturer", "Invalid Manufacturer");
			pass = false;
		}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			try {
				VaccineBean bean = model.findByPK(id);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SAVE.equalsIgnoreCase(op)) {
			...
			try {
				...
				ServletUtility.setBean(bean, request);
				ServletUtility.setErrorMessage("Vaccine Already Exists", request);
				e.printStackTrace();
			} catch (ApplicationException e) {
				ServletUtility.handleException(e, request, response);
				e.printStackTrace();
			...
			try {
				...
				ServletUtility.setBean(bean, request);
				ServletUtility.setErrorMessage("Vaccine Name already exists", request);
			} catch (ApplicationException e) {
				e.printStackTrace();
				ServletUtility.handleException(e, request, response);
				return;
